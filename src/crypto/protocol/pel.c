...
void pel_setup_context( struct pel_context *pel_ctx,
                        const char *key, unsigned char IV[20] );
...
int pel_server_init( int server, const char *key,
                     struct pel_context *send_ctx, struct pel_context *recv_ctx )
{
    ...
    unsigned char buffer[PEL_BUFFER_SIZE];
    ...
    ret = pel_send_msg( server, challenge, 16, send_ctx, buffer );

    if( ret != PEL_SUCCESS ) return( PEL_FAILURE );

    /* handshake - decrypt and verify the server's challenge */

    ret = pel_recv_msg( server, buffer, &len, recv_ctx, buffer );
...
/* session setup */
int pel_client_init( int client, const char *key,
                     struct pel_context *send_ctx, struct pel_context *recv_ctx )
{
    int ret, len;
    unsigned char IV1[20], IV2[20];
    ...
    ret = pel_recv_msg( client, buffer, &len, recv_ctx, buffer );
    ...
    ret = pel_send_msg( client, challenge, 16, send_ctx, buffer );
                        ...
                        const char *key, unsigned char IV[20] )
{
    ...
    sha1_finish( &sha1_ctx, key_material );

    aes_set_key( &pel_ctx->SK, key_material, 128 );
    ...
    for( i = 0; i < 20; i++ )
    {
        pel_ctx->k_ipad[i] ^= key_material[i];
        pel_ctx->k_opad[i] ^= key_material[i];
    }
...
int pel_send_all( int s, void *buf, size_t len, int flags )
{
    ...
    while( sum < len )
    {
        ...
        n = select(s + 1, NULL, &writefds, NULL, &timeout);
        if( n <= 0 )
        {
            /* 超时或错误 */
            pel_errno = PEL_SYSTEM_ERROR;
